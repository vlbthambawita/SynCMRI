...
def add_segmentations_to_noise(noisy_images, segmentations_batch, config, device):
    ...
    if config.segmentation_channel_mode == "single":
        ...
        # Find the segmentation tensor (starts with 'seg_')
        segs = [v for k, v in segmentations_batch.items() if k.startswith("seg_")][0]
        segs = segs.to(device)
        # Concatenate: [Batch, Image_Channels + Mask_Channels, H, W]
        noisy_images = torch.cat((noisy_images, segs), dim=1)
...
class SegGuidedDDPMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        """
        # The number of clean image channels is simply the model's output channels (1)
        ...
        if isinstance(self.unet.config.sample_size, int):
            ...
            # 3. Fix the else block to ALSO use the variable
...
class SegGuidedDDIMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # Use that variable directly (No more math inside the if/else!)
        if isinstance(self.unet.config.sample_size, int):
            image_shape = (
                batch_size,
                img_channel_ct,  # <--- WE USE THE VARIABLE HERE
                self.unet.config.sample_size,
                self.unet.config.sample_size,
            )
        else:
